import { ReflectChildrenMixin, ReflectSceneNode, ReflectSceneNodeType } from "@bridged.xyz/design-sdk/lib/nodes";
import { CheckResult } from "..";
import { Allowence, StructureRule } from "../rules";


export function checkIfValidStructure(node: ReflectSceneNode, rule: StructureRule): CheckResult {
    ...
    const valid = validNodeType &&
        validChildrenNodeTypes
...
function checkIfValidNodeType(type: ReflectSceneNodeType, allowence: Allowence<ReflectSceneNodeType>[]): boolean {
    const config = allowence.find((a) => { return a.target === type })
    if (config) {
        return config.allow
    } else {
        // config not found
        const deafaultConfig = allowence.find((a) => { return a.target === '*' })
